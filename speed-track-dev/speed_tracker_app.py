...
class SplashScreen(ctk.CTkToplevel):
    def __init__(self, master):
        ...
        ctk.CTkLabel(main_frame, text="SpeedTracker Dev", font=("Helvetica", 24, "bold")).pack(pady=10)
        self.status_label = ctk.CTkLabel(main_frame, text="Carregando componentes...", font=("Helvetica", 12))
...
class SpeedTrackerApp(ctk.CTk):
    def __init__(self):
        ...
        if sys.platform == "win32":
            try:
                import pywinstyles; pywinstyles.apply_style(self, "mica")
    ...
    def jump_to_task(self, index, event=None):
        ...
        self.pause_button.configure(text="Pausar", state=tk.NORMAL); self.copy_button.configure(state=tk.NORMAL); self.start_button.configure(state=tk.DISABLED)
    ...
    def task_finished_handler(self):
        ...
        if self.current_task_index != -1:
            self.progress[self.current_task_index] = True; self.update_task_status(self.current_task_index, "completed")
            self.log_activity(self.tasks[self.current_task_index]); self.save_progress()
    ...
    def copy_task_to_clipboard(self): task_text = self.task_label.cget("text"); self.clipboard_clear(); self.clipboard_append(task_text); self.copy_button.configure(text="Copiado!", state=tk.DISABLED); self.after(2000, self.reset_copy_button_text)
    def reset_copy_button_text(self): self.copy_button.configure(text="Copiar Enunciado", state=tk.NORMAL)
    ...
    def reset_app_state(self):
        ...
        self.pause_button.configure(text="Pausar", state=tk.DISABLED); self.copy_button.configure(state=tk.DISABLED)
    def start_roteiro(self): self.start_button.configure(state=tk.DISABLED); self.find_and_start_next_pending()
...
if __name__ == "__main__":
    if sys.platform == "win32":
        try:
            from ctypes import windll; myappid = 'speedtracker.dev.python.gui.1'
